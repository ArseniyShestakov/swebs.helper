...
```
_Рассчитывает скидку для корзины текущего пользователя, включая правила работы с корзиной. если передан купон, то перед рассчётом он будет активированы. Возвращает массив с общей скидочной ценой корзины и по каждому товару отдельно._
...
## Swebs\Helper\Main\GarbageStorage
```php
